...
{--
getMarkup :: WebDriver wd => wd TL.Text
getMarkup = do
    markup <- executeJS [] "return document.getElementsByTagName('html')[0].innerHTML;"
...
updateComicTableChapter comic chapter_@(Chapter chapter section) = do
...
openAndScanComic = do
    ...
    runSimpleApp . logInfo . display $
        vivid Black <> "scanning "
            <> (vivid Yellow <> title)
            <> ( vivid Black <> " (Vol." <> T.pack (show volumeLast)
...
--         a[href='https://manga1000.top/page/2'],
--         a[href='/page/2'],
--         a[href='page/2'],
--         a[href='https://manga1000.top/page/2/'],
--         a[href='/page/2/'],
...
clickAnchor :: URI -> WD ()
clickAnchor url = do
    anchor <- findElem $ ByCSS $ selectorAnchor url
    click anchor
--}
