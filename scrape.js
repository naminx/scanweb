...
const xpath = (path, root = document) =>
  document.evaluate(path, root, null, XPathResult.FIRST_ORDERED_NODE_TYPE)
    .singleNodeValue;
...
return xpaths('//div[contains(@class, "chapters-container")]//a')
  .reverse()
  .map((a) => ({
...
return xpaths('//div[contains(@class, "card-wrap")]/img').map(
  (img) => img.dataset.src
);
...
//============================================================================
return xpaths(
  '//div[contains(@class, "card-body")]/div[contains(@class, "rotate-img")]/a'
).map((a) => ({
  url: a.href,
}));
...
return xpaths('//div[@class="list-scoll"]//a')
  .reverse()
  .map((a) => ({
    chapter: a.title.replace(/.*【第(.*)話】.*/, "$1"),
    url: a.href,
...
return xpaths('//div[@class="chapterlist"]//a')
  .reverse()
  .map((a) => ({
    chapter: a.title.replace(/.*【第(.*)話】.*/, "$1"),
    url: a.href,
...
return xpaths('//div[@class="entry-content"]//img').map(
  (img) => img.dataset.src
);
...
return xpaths(
  ...
  chapter: xpath(
    'a//following-sibling::div/div[contains(@class, "chapter-title")]',
    div
  ).title.replace(/^Chap /, ""),
}));
...
return xpaths('//ul[contains(@class, "list-chapters")]/a')
  .reverse()
  .map((a) => ({
...
return xpaths(
  ...
  url: xpath('following-sibling::div[contains(@class, "series-title")]/a', div)
    .href,
  chapter: xpath(
    'a//following-sibling::div/div[contains(@class, "chapter-title")]',
    div
...
return xpaths('//div[contains(@class, "thumb-item-flow")]').map((div) => ({
  ...
  chapter: xpath('div/div/div[contains(@class, "chapter-title")]/a', div)
    .textContent,
}));
...
return xpaths('//div[@class="chapter-content"]//img').map(
  (img) => img.dataset.src
);
...
return xpaths('//div[@id="tab-chapper")//a')
  .reverse()
  .map((a) => ({
    chapter: a.title.replace(/^Chapter /, ""),
    url: a.href,
...
// hachimanga.com
//============================================================================
return xpaths('//div[contains(@class, "item-summary")]').map((div) => {
  const url = xpath('div[contains(@class, "post-title")]/h3/a', div).href;
  const release = xpath(
    'div[contains(@class, "list-chapter")]/div[contains(@class, "chapter-item")]//a',
    div
  ).textContent;
...
return xpaths('//div[contains(@class, "listing-chapters_wrap")]/ul/li/a')
  .reverse()
  .map((a) => {
    const release = a.textContent;
    if (release.match(/.*第(.*)巻.*/)) {
      return {
        volume: release.replace(/.*第(.*)巻.*/, "$1").trim(),
        url: a.href,
      };
    } else {
      return {
        chapter: release.replace(/.*第(.*)話.*/, "$1").trim(),
        url: a.href,
      };
    }
  });
...
return xpaths(
  '//div[contains(@class, "read-container")]/div[contains(@class, "reading-content")]//img'
).map((img) => img.dataset.src.trim());
...
return xpaths('//ul[@class="clstyle"]/li')
  .reverse()
  .map((li) => ({
...
return xpaths('//table[@id="chapter_table"]//a[@class="chico"]')
  .reverse()
  .map({
